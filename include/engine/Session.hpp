...
namespace fix::engine
{
  ...
  public:
    ...
    void on_frame(std::string_view frame)
    {
      ...
      if (res.status == fix::core::Parser::Status::NeedMore)
        return; // not expected here
      if (res.status == fix::core::Parser::Status::BadFrame)
      {
        h_.on_bad_frame(res.error, res.error_tag);
        return;
      }
      ...
      if (mt == Logon)
        return h_.on_logon({m});
      if (mt == Logout)
        return h_.on_logout({m});
      if (mt == Heartbeat)
        return h_.on_heartbeat({m});
      if (mt == TestRequest)
        return h_.on_test_request({m});
      if (mt == ResendRequest)
        return h_.on_resend_request({m});
      if (mt == Reject)
        return h_.on_reject({m});
      if (mt == SequenceReset)
        return h_.on_sequence_reset({m});
