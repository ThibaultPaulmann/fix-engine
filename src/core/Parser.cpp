...
namespace fix::core
{
  ...
  Parser::Result Parser::parse(std::string_view buf, Message &out)
  {
    ...
    {
      if (e9 < pos9 + 2)
        return {Status::BadFrame, "empty BodyLength", BodyLength, 0, {}};
      std::string_view digits{buf.data() + pos9 + 2, e9 - (pos9 + 2)};
      auto opt = sv_to_int(digits);
      if (!opt)
        return {Status::BadFrame, "invalid BodyLength", BodyLength, 0, {}};
      bodyLen = *opt;
      if (bodyLen < 0)
        return {Status::BadFrame, "negative BodyLength", BodyLength, 0, {}};
    }
    ...
    int expected = 0;
    {
      ...
      if (!opt)
        return {Status::BadFrame, "invalid CheckSum digits", CheckSum, 0, {}};
      expected = *opt;
    }
    if (sum != expected)
    {
    ...
    while (cur < bodyEnd)
    {
      ...
      if (eq == std::string_view::npos || eq >= bodyEnd)
        break;
      const auto soh = buf.find(SOH, eq);
      if (soh == std::string_view::npos || soh > bodyEnd)
        break;

      std::string_view tsv{buf.data() + cur, eq - cur};
      auto ti = sv_to_int(tsv);
      if (!ti)
        return {Status::BadFrame, "non-numeric tag", 0, 0, {}};
      const int tag = *ti;
