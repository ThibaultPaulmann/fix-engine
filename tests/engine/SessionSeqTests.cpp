...
int main() {
  ...
  auto logon = fix::test::build_frame({
    {BeginString,"FIXT.1.1"},{MsgType,"A"},{SenderCompID,"C1"},{TargetCompID,"S1"},
    {MsgSeqNum,"1"},{SendingTime,"20250101-12:00:00.000"},{EncryptMethod,"0"},{HeartBtInt,"30"},{DefaultApplVerID,"9"}
  });
  s.on_frame(logon);
  auto hb2 = fix::test::build_frame({
    {BeginString,"FIXT.1.1"},{MsgType,"0"},{SenderCompID,"C1"},{TargetCompID,"S1"},
  ...
  auto hb5 = fix::test::build_frame({
    {BeginString,"FIXT.1.1"},{MsgType,"0"},{SenderCompID,"C1"},{TargetCompID,"S1"},
    {MsgSeqNum,"5"},{SendingTime,"20250101-12:00:01.000"}
  });
  s.on_frame(hb5);
  if (h.gaps != 1) { std::cerr<<"gap detect failed\n"; return EXIT_FAILURE; }

  // 3) PossDup: seq lower than expected with 43=Y
  auto hb_dup = fix::test::build_frame({
    {BeginString,"FIXT.1.1"},{MsgType,"0"},{SenderCompID,"C1"},{TargetCompID,"S1"},
    {MsgSeqNum,"2"},{SendingTime,"20250101-12:00:01.500"},{PossDupFlag,"Y"}
  });
  s.on_frame(hb_dup);
  if (h.possdups != 1) { std::cerr<<"possdup failed\n"; return EXIT_FAILURE; }

  // 4) Too low (without PossDup): seq lower than expected & no 43=Y
  auto hb_low = fix::test::build_frame({
    {BeginString,"FIXT.1.1"},{MsgType,"0"},{SenderCompID,"C1"},{TargetCompID,"S1"},
    {MsgSeqNum,"1"},{SendingTime,"20250101-12:00:02.000"}
  });
  s.on_frame(hb_low);
  if (h.lows != 1) { std::cerr<<"low seq failed\n"; return EXIT_FAILURE; }
