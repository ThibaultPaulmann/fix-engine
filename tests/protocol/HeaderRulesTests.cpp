...
int main()
{
  ...
  auto frame_ok = fix::test::build_frame({{BeginString, "FIXT.1.1"},
                                          {MsgType, "0"},
                                          {SenderCompID, "C1"},
                                          {TargetCompID, "S1"},
                                          {MsgSeqNum, "1"},
  ...
  if (!v.empty())
  {
    std::cerr << "expected 0 violations, got " << v.size() << "\n";
    for (auto &vi : v)
    {
      std::cerr << "  tag=" << vi.tag
                << " what=" << (vi.what ? vi.what : "<null>")
                << "\n";
    }
    return EXIT_FAILURE;
  }
  ...
  auto frame_miss = fix::test::build_frame({{BeginString, "FIXT.1.1"},
                                            {MsgType, "0"},
  ...
  if (!found)
  {
    std::cerr << "expected violation for SenderCompID\n";
    return EXIT_FAILURE;
  }
