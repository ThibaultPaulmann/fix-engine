...
int main() {
  ...
  if (m.get_sv(MsgType).value_or("") != "A") return EXIT_FAILURE;
  ...
  if (pos10 != std::string::npos && pos10+3 < bad.size()) {
    ...
    if (bad[pos10+3] >= '0' && bad[pos10+3] <= '8') bad[pos10+3] += 1;
    else if (bad[pos10+3] == '9') bad[pos10+3] = '0';
  }
